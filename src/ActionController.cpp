...
void ActionController::obstacleStatus(SensorDate loc) {

    //  ====================================== 초기 및 측면 회피 로직 ======================================
    // 1-1(번호 수정). 정상 청소 중 전방 장애물 최초 감지 
    if (!isEvading && loc.frontSensorDate && !isMovingBackwald){
        std::cout << "[ActionController] 전방 장애물 감지! 회피 시작...\n";
        
        isEvading = true; // 회피 시작.
        checkingRight = true; // 추가된 로직, 오른쪽 장애물 확인을 위함.

        location.isLeftBlocked = loc.leftSensorDate;
        location.isFrontBlocked = loc.frontSensorDate;
    ...
    if (isEvading && checkingRight && !isMovingBackwald) {
        checkingRight = false;

        location.isFrontBlocked=loc.leftSensorDate;
        location.isRightBlocked=loc.frontSensorDate;
    ...
    //  ====================================== 후진 로직 ======================================
    // 2-1 후진 중 센서 수신
    if (isEvading && isMovingBackwald && !checkingRight){
        checkingRight = true; // 추가된 로직, 오른쪽 장애물 확인을 위함.
